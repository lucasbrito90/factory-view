...
<template>
  <v-table class="bordered-table" hover density="comfortable">
    <thead class="bg-containerBg">
      <tr>
        <th class="text-left pl-6 text-caption font-weight-bold text-uppercase">#</th>
        ...
        <th class="text-right pr-6 text-caption font-weight-bold text-uppercase">Average</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="item in products" :key="item.name">
        ...
        <td style="min-width: 150px">{{ item.country }}</td>
        <td style="min-width: 150px">{{ item.name }}</td>
        <td class="text-right pr-6">{{ item.average }}%</td>
      </tr>
    </tbody>
  </v-table>
</template>
