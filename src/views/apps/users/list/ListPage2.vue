...
import type { Phone, User } from '@/context/enrollment/interfaces/user';
...
const getPrimaryOrFirstPhone = (phones: Phone[] | undefined): string | undefined => {
  const primaryPhone = phones?.find((phone) => phone.is_primary === true);

  if (primaryPhone) {
    return `+${primaryPhone.country_code} ${primaryPhone.number} ${primaryPhone.extension}`;
  }

  if (phones && phones.length > 0) {
    return `+${phones[0].country_code} ${phones[0].number} ${phones[0].extension}`;
  }
      ...
      <UiParentCard :title="$t('User List')">
        ...
        <v-row v-for="card in listCards" :key="card.id">
          <v-col md="3" sm="5" cols="12" class="mt-2">
            <div class="d-flex align-start ga-4">
              ...
              <div class="d-flex align-center ga-2" v-if="!card.avatar">
                <v-avatar size="32" color="lightprimary" class="text-primary" variant="flat">
                  {{ card.name && card.name.substring(0, 2).toLocaleUpperCase() }}
                </v-avatar>
              </div>
              <div>
          ...
          <v-col md="2" sm="3" cols="6">
            ...
            <template v-if="card.phones && card.phones?.length > 0">
              <small class="mt-2 d-block font-weight-bold">{{ $t('PersonalInformation.Phone Number') }}</small>
              <h5 class="text-h6">{{ getPrimaryOrFirstPhone(card.phones) }}</h5>
            </template>
          </v-col>
          <v-col md="2" sm="3" cols="6">
