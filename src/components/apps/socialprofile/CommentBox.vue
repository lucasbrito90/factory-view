    ...
    <div class="d-flex ga-4 align-center">
      ...
      <v-menu rounded="md">
        <template v-slot:activator="{ props }">
          <v-btn icon size="small" aria-label="menu" variant="text" color="secondary" class="ml-auto" rounded="md" v-bind="props"
    ...
    <div v-for="reply in comments?.data.replies" :key="reply.name">
      <v-card variant="flat" elevation="0" rounded="lg" class="bg-containerBg mb-3 pa-5">
        <div class="d-flex ga-4 align-center">
          ...
          <v-menu rounded="md">
            <template v-slot:activator="{ props }">
              <v-btn icon size="small" aria-label="menu" rounded="md" variant="text" color="secondary" class="ml-auto" v-bind="props"
                ><SvgSprite name="custom-more-outline" style="width: 18px; height: 18px; transform: rotate(90deg)"
              /></v-btn>
            </template>
