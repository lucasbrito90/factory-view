...
const photo: Ref<File | undefined> = ref(undefined);
const imageURL: Ref<string> = ref('');

onMounted(() => {
  if (user.userAuthenticated?.avatar) {
    imageURL.value = user.userAuthenticated.avatar as unknown as string;
  }
});
...
function onSelectedPhoto(event: Event) {
  photo.value = (event.target as HTMLInputElement).files?.[0] || undefined;

  if (user.userAuthenticated !== null && user.userAuthenticated !== undefined) {
    user.userAuthenticated.avatar = photo.value;
  }
...
<template>
  <div class="text-center">
    <v-avatar size="124" variant="outlined" color="primary" class="dashed bg-lightprimary" v-if="user.userAuthenticated?.avatar">
    ...
    <v-avatar size="124" color="lightprimary" class="text-primary" variant="flat" v-if="!user.userAuthenticated?.avatar">
      <span class="text-h1">{{user.userAuthenticated?.name?.substring(0, 2).toUpperCase() || ''}}</span>
      <input type="file" class="preview-upload" aria-label="upload file" @change="onSelectedPhoto"/>
    </v-avatar>
    <h5 class="text-h5 pt-5 mb-1">{{ user.userAuthenticated?.name || '' }}</h5>
    <p class="text-h6 text-lightText">{{ user.userAuthenticated?.role || '' }}</p>
